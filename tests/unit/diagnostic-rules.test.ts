...
describe("diagnostic rules", () => {
  it("marks none and partial answers as weak formulas once", () => {
    ...
    assert.deepEqual(weakFormulaIds, ["f1"]);
  });

  it("keeps the strongest assessment for each formula", () => {
    const assessments = calculateBestDiagnosticAssessmentsByFormula({
      reviewItems,
      answers: [
        { reviewItemId: "r1", assessment: "none" },
