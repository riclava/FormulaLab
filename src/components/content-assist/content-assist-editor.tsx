...
export function ContentAssistEditor({
  ...
  async function saveDraft() {
    ...
    try {
      const response = await fetch(`/api/content-assist/drafts/${draft.formulaSlug}`, {
        method: "PATCH",
        headers: {
  ...
  async function regenerateDraft() {
    ...
    try {
      const response = await fetch("/api/content-assist/generate", {
        ...
        body: JSON.stringify({
          formulaIdOrSlug: draft.formulaSlug,
        }),
      });
      const payload = (await response.json()) as {
        data?: ContentAssistDraft;
  ...
  async function approveDraft() {
    ...
    try {
      const saveResponse = await fetch(
        `/api/content-assist/drafts/${draft.formulaSlug}`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
      ...
      const approveResponse = await fetch(
        `/api/content-assist/drafts/${draft.formulaSlug}/approve`,
        {
          method: "POST",
        },
      );
      ...
      <section className="grid gap-6 lg:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)]">
        <div className="grid gap-6">
          <EditableSection title="解释草稿" eyebrow="Explanation">
            <TextField
              label="一句话用途"
              value={draft.explanation.oneLineUse}
              onChange={(value) =>
                setDraft((current) => ({
                  ...current,
                  explanation: {
            ...
            <TextAreaField
              label="意义说明"
              value={draft.explanation.meaning}
              onChange={(value) =>
                setDraft((current) => ({
                  ...current,
                  explanation: {
