  ...
  if (questions.length === 0) {
    return (
      <section className="flex flex-col gap-4 rounded-lg border bg-background p-6 shadow-sm">
        ...
        <h2 className="text-xl font-semibold">还没有可用于诊断的公式内容。</h2>
        <p className="text-sm text-muted-foreground">
          请先执行种子脚本，写入概率统计 Alpha 公式集。
        </p>
      </section>
    );
  }
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                {diagnostic.domain} · {currentQuestion.formula.title}
              </p>
            </div>
          </div>
  ...
  return (
    <div className="flex flex-col gap-3 rounded-lg border bg-muted/50 p-4">
      ...
      {question.type === "recall" ? (
        <LatexRenderer block expression={question.formula.expressionLatex} />
      ) : (
...
function DiagnosticResultView({ result }: { result: DiagnosticResult }) {
  const weakCount = result.weakFormulas.length;
    ...
    <section className="flex flex-col gap-6 rounded-lg border bg-background p-6 shadow-sm">
      ...
      {weakCount > 0 ? (
        <div className="grid gap-3 md:grid-cols-2">
          {result.weakFormulas.map((formula) => (
